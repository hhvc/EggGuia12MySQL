...
select * from partidos where puntos_local -puntos_visitante = (select abs(max(puntos_local-puntos_visitante)) from partidos);

-- 16. Mostrar la media de puntos en partidos de los equipos de la división Pacific.
SELECT AVG(PUNTOS_LOCAL + PUNTOS_VISITANTE) AS Puntos_Promedio FROM PARTIDOS WHERE equipo_local OR EQUIPO_VISITANTE in (select nombre FROM EQUIPOS WHERE DIVISION = 'PACIFIC');

-- 17. Mostrar los puntos de cada equipo en los partidos, tanto de local como de visitante.
SELECT 
    T1.EQUIPO, PUNTOS_LOCAL + PUNTOS_VISITANTE PUNTOS
FROM
    (SELECT 
    ...
    FROM
        PARTIDOS
    GROUP BY EQUIPO_VISITANTE) T2 ON T1.EQUIPO = T2.EQUIPO;
